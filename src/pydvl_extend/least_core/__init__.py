...
from pydvl_extend.least_core.pruned import pruned_montecarlo_least_core, NoPruning
...
def compute_least_core_values(
    ...
    **kwargs,
    ...
    elif mode == LeastCoreMode.PrunedMontecarlo:
        pruning = kwargs.pop("pruning", NoPruning())
        # TODO fix progress showing and maybe_progress in remote case
        progress = False
        if n_iterations is None:
            raise ValueError("n_iterations cannot be None for Monte Carlo Least Core")
        return pruned_montecarlo_least_core(
            u=u,
            n_iterations=n_iterations,
            n_jobs=n_jobs,
            progress=progress,
