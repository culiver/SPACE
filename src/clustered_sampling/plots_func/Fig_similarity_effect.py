...
def get_metric_evo(
    ...
    try:
        ...
        pass
    if idx_row == 0:
        axis[idx_row].set_ylabel("Global Loss", labelpad=0.0)
    elif idx_row == 1:
        axis[idx_row].set_ylabel("Global Loss")
    if idx_row == n_rows - 1:
        axis[idx_row].set_xlabel("# rounds", labelpad=0.0)
    #    if idx_row ==0: axis[idx_row].set_ylim(40)
    if idx_row == 1:
        axis[idx_row].legend(kept_legend, ncol=2, title=r"$\bf{Sampling}$")


def plot_fig_similarity(alpha: float, n_SGD: int, p: float, lr: float):

    kept = ["MD", "Alg. 1", "Alg. 2 Arccos", "Alg. 2 L1", "Alg. 2 L2"]
    ...
    plt.savefig(f"plots/plot_similarity.pdf")
