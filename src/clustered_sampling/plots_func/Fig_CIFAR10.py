...
def metric_evo(
    ...
    def plot_hist_mean(hist: np.array, weights, axis, idx_row, idx_col, smooth):
        ...
        if smooth:
            ...
            return axis[idx_col].plot(a, ysmooth)
        else:
            return axis[idx_col].plot(X, y)

    # weights = weights_clients(dataset)
    if metric == 'loss':
        weights = np.array([100] * 10 + [250] * 30 + [500] * 30 + [750] * 20 + [1000] * 10)
    ...
    if idx_row == n_rows - 1:
        axis[idx_col].set_xlabel("# rounds")
    if idx_col == 0:
        if metric=='acc':
            axis[idx_col].set_ylabel("Accuracy (%)")
        else:
            axis[idx_col].set_ylabel("Loss")
...
def plot_fig_CIFAR10_alpha_effect_one(
    ...
    # kept = ["MD", "Alg. 1", "Alg. 2", "Alg. 3", "Alg. 4"]
    kept = ["MD", "CS(Alg.1)", "CS(Alg.2)", "CS(SPACE)"]
