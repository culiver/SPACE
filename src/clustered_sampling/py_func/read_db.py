...
def get_dataloaders(train_dataset, test_dataset, user_groups, batch_size: int, shuffle=True):
    from update import DatasetSplit
    n_clients = 100

    list_dls_train = [
        torch.utils.data.DataLoader(DatasetSplit(train_dataset, user_groups[i]), batch_size=batch_size, shuffle=True)
