...
def get_gradients(sampling, global_m, local_models):
    ...
    for model in local_models:
        local_model_params += [
            [tens.cpu().detach().numpy() for tens in list(model.parameters())]
        ]

    global_model_params = [
        tens.cpu().detach().numpy() for tens in list(global_m.parameters())
    ]
...
def get_matrix_similarity_from_prototypes(local_model_prototypes, distance_type):
    """return the similarity matrix where the distance chosen to
    compare two clients is set with `distance_type`"""

    n_clients = len(local_model_prototypes)

    metric_matrix = np.zeros((n_clients, n_clients))
    for i, j in product(range(n_clients), range(n_clients)):

        metric_matrix[i, j] = get_similarity(
            local_model_prototypes[i], local_model_prototypes[j], distance_type
        )
